-- ============================================
-- Migration 009: Fix TickCount Frequency Calculation
-- Adds p_tick_frequency parameter for accurate Stopwatch tick conversion
-- ============================================
-- Date: December 1, 2025
-- Issue: offline_duration_minutes calculation was using wrong divisor (60000 assumes milliseconds)
-- Fix: Use Stopwatch.Frequency from C# to correctly convert ticks to seconds
-- ============================================
...
CREATE PROCEDURE sp_record_attendance_scan_secure(
    ...
    IN p_tick_frequency BIGINT,           -- NEW: Stopwatch.Frequency (ticks per second)
...
sp_label: BEGIN
    ...
    IF v_existing_scan_id IS NOT NULL AND v_existing_time_out IS NULL THEN
        ...
        -- ✅ FIXED: Calculate offline duration using Stopwatch.Frequency
        -- Converts ticks to seconds, then to minutes
        -- Formula: (tick_count_out - tick_count_in) / frequency / 60.0
...
-- Verification
-- ============================================
SELECT 'Migration 009 completed - Stored procedure updated with tick frequency parameter' AS status;
