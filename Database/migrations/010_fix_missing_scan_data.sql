-- ============================================
-- Migration 010: Fix Missing scan_data Field
-- Adds missing scan_data field to INSERT statement
-- ============================================
-- Date: December 1, 2025
-- Issue: Error 1364 - Field 'scan_data' doesn't have a default value
-- Fix: Add scan_data to the INSERT statement in sp_record_attendance_scan_secure
-- ============================================
...
sp_label: BEGIN
    ...
    IF v_existing_scan_id IS NOT NULL AND v_existing_time_out IS NULL THEN
        ...
        -- Calculate offline duration using Stopwatch.Frequency
    ...
    ELSE
        ...
        -- ✅ FIXED: Insert new Time-In record with scan_data field
        INSERT INTO scan_history (
            student_id,
            device_id,
            scan_data,               -- ✅ ADDED: Required field
            scan_type,               -- ✅ ADDED: Set to 'QR'
            scan_datetime,
            time_in_validation_mode,
            time_in_tick_count,
            validation_status,       -- ✅ ADDED: Validation status field
            client_time,             -- ✅ ADDED: Client time tracking
            server_time,             -- ✅ ADDED: Server time tracking
            time_drift_seconds,      -- ✅ ADDED: Time drift tracking
            connection_drop_count,   -- ✅ ADDED: Connection tracking
            status,
            notes,
            requires_review,
            location
            ...
            v_student_id,
            p_device_id,
            p_scan_data,             -- ✅ ADDED: Store the QR code data
            'QR',                    -- ✅ ADDED: Scan type is QR
            NOW(),
            v_current_validation_mode,
            p_tick_count,
            p_validation_status,     -- ✅ ADDED: Store validation status
            p_client_time,           -- ✅ ADDED: Store client time
            p_server_time,           -- ✅ ADDED: Store server time
            p_time_drift_seconds,    -- ✅ ADDED: Store time drift
            p_connection_drop_count, -- ✅ ADDED: Store connection drops
            v_status,
            v_notes,
            (v_status = 'for_review'),
            p_location
...
-- Verification
-- ============================================
SELECT 'Migration 010 completed - Fixed missing scan_data field in INSERT statement' AS status;
